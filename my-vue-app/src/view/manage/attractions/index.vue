 ...
 <template>
  <div>
   <el-table :data="goods.slice((currentPage-1)*pageSize,currentPage*pageSize)" border @row-click="viewDetails" >
    ...
    <el-table-column prop="title" label="地点" />
    <el-table-column prop="area" label="地区" width="250" />
    <el-table-column prop="product" label="热度" width="100" />
    <el-table-column prop="address" label="地址" width="250" />
    <el-table-column prop="price" label="价格" width="100" />
    <el-table-column prop="amount" label="评论数" width="100" />
   ...
   <el-dialog v-model="dialogVisible1" title="景点修改" width="60%" draggable center="true" align-center="true" @close="closeDialog">
     <el-form ref="GoodsFormRef" :model="GoodsForm" :rules="GoodsFormRules" label-width="0px">
       <el-form-item prop="title">
         <span>地点名称</span>
         <el-input type="text" v-model="GoodsForm.title"  placeholder="输入新的地点名称" clearable >
         </el-input>
       </el-form-item>
       <el-form-item prop="level">
        <span>等级</span>
        <el-input type="text" v-model="GoodsForm.level"  placeholder="输入新的等级" clearable >
        </el-input>
       </el-form-item>
       <el-form-item prop="area">
       <span>地区</span>
       <el-input type="text" v-model="GoodsForm.area"  placeholder="输入新的地区" clearable >
       </el-input>
       </el-form-item>
       ...
       <el-form-item prop="product">
        <span>热度</span>
        <el-input type="text" v-model="GoodsForm.product"  placeholder="输入新的热度" clearable >
        </el-input>
       </el-form-item>
      ...
      <span>信息</span>
      <el-input type="text" v-model="GoodsForm.information"  placeholder="输入新的信息" clearable >
      </el-input>
       </el-form-item>
       <el-form-item prop="amount">
     <span>评论数</span>
     <el-input type="text" v-model="GoodsForm.amount"  placeholder="输入新的评论数" clearable >
     </el-input>
       </el-form-item>
     </el-form>
   ...
   <el-dialog
     ...
     @close="closeDialog2"
   > 
     <div class="delete">地点序号：{{removeData[0].id}}</div>
     <div class="delete">地点名称：{{removeData[0].title}}</div>
     <div class="delete">热度：{{removeData[0].product}}</div>
     <div>
       <img :src="removeData[0].image" width="200" style="border-radius: 5px;">
     </div>
     <div style="color: red;font-size:18px;text-align:center;width:100%;margin-top:10px;">确认删除此景点吗</div>
     <template #footer>
       <span class="dialog-footer">
 ...
 >
   <div class="dialog">
     <div class="left">
       ...
       <div>名字：{{details.title}}</div>
       <div>地区：{{details.area}}</div>
       <div>{{details.product}}</div>
       <div>{{details.address}}</div>
       <div>价格：{{details.price}}</div>
       <div>评论数：{{details.amount}}</div>
     </div>
