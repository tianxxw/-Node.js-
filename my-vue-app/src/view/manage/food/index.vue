 ...
 <template>
  <div>
   <el-table :data="goods.slice((currentPage-1)*pageSize,currentPage*pageSize)" border @row-click="viewDetails" >
    ...
    <el-table-column prop="name" label="美食" width="100" />
    <el-table-column prop="link" label="美食链接"  >
    ...
    <el-table-column prop="address" label="店铺名" width="250" />
    <el-table-column prop="area" label="店铺链接" width="250" >
     <template #default="link">
      <div>
       <a class="link" target="_blank" :href="link.row.area">{{link.row.area}}</a>
      </div>
     </template>
    </el-table-column>
    <el-table-column prop="number" label="推荐量" width="100" />
   ...
   <el-dialog v-model="dialogVisible1" title="美食修改" width="60%" draggable center="true" align-center="true" @close="closeDialog">
     <el-form ref="GoodsFormRef" :model="GoodsForm" :rules="GoodsFormRules" label-width="0px">
       <el-form-item prop="name">
         <span>美食名称</span>
         <el-input type="text" v-model="GoodsForm.name"  placeholder="输入新的美食名称" clearable >
         </el-input>
       </el-form-item>
       <el-form-item prop="link">
        <span>美食链接</span>
        <el-input type="text" v-model="GoodsForm.link"  placeholder="输入新的美食链接" clearable >
        </el-input>
       </el-form-item>
       <el-form-item prop="address">
        <span>店铺名</span>
        <el-input type="text" v-model="GoodsForm.address"  placeholder="输入新的价格" clearable >
        </el-input>
      </el-form-item>
       <el-form-item prop="area">
       <span>店铺链接</span>
       <el-input type="text" v-model="GoodsForm.area"  placeholder="输入新的店铺链接" clearable >
       </el-input>
       </el-form-item>
       <el-form-item prop="number">
        <span>推荐量</span>
        <el-input type="text" v-model="GoodsForm.number"  placeholder="输入新的推荐量" clearable >
        </el-input>
        </el-form-item>
     </el-form>
   ...
   <el-dialog
     ...
     @close="closeDialog2"
   > 
     <div class="delete">美食序号：{{removeData[0].id}}</div>
     <div class="delete">美食名称：{{removeData[0].name}}</div>
     <div class="delete">店铺名称：{{removeData[0].address}}</div>
     <div class="delete">美食所获推荐量：{{removeData[0].number}}</div>
     <div>
       <img :src="removeData[0].image" width="200" style="border-radius: 5px;">
     </div>
     <div style="color: red;font-size:18px;text-align:center;width:100%;margin-top:10px;">确认删除此美食吗</div>
     <template #footer>
       <span class="dialog-footer">
 ...
 >
   <div class="dialog">
     <div class="left">
       ...
       <div>美食：{{details.name}}</div>
       <div>美食链接：{{details.link}}</div>
       <div>店铺：{{details.address}}</div>
       <div>店铺链接：{{details.area}}</div>
       <div>美食所获推荐量：{{details.number}}</div>
     </div>
